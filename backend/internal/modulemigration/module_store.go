...
func newSQLModuleStore(db *sql.DB) *sqlModuleStore { return &sqlModuleStore{db: db} }
...
func (s *sqlModuleStore) GetInstalled(ctx context.Context, id string) (*modules.InstalledModule, error) {
	return scanInstalledModule(s.db.QueryRowContext(ctx, `SELECT id,name,type,version,status,install_path,manifest_json,installed_at,enabled_at,COALESCE(last_error,'') FROM installed_modules WHERE id=$1`, id))
}
func (s *sqlModuleStore) SaveInstalled(ctx context.Context, m modules.InstalledModule) error {
...
func (s *sqlModuleStore) SavePermissions(ctx context.Context, moduleID string, perms []modules.PermissionRecord) error {
...
func (s *sqlModuleStore) SetStatus(ctx context.Context, id string, status modules.ModuleStatus, lastErr string) error {
	var enabled any
...
type installedScanner interface{ Scan(...any) error }

func scanInstalledModule(row installedScanner) (*modules.InstalledModule, error) {
