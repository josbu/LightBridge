...
func (s *SchedulerSnapshotService) rebuildByAccount(ctx context.Context, account *Account, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	if account.IsCustom() {
		// Custom 账号（platform=="custom"，上面已重建 custom bucket）会出现在其所属分组的
		// 「protocol 对应原生平台」候选集中，故需一并重建该原生平台的 buckets。
		var nativePlatform string
		switch account.CustomProtocol() {
		case CustomProtocolAnthropicMessages:
			nativePlatform = PlatformAnthropic
		case CustomProtocolGemini:
			nativePlatform = PlatformGemini
		case CustomProtocolOpenAIResponses, CustomProtocolOpenAIChatCompletions, CustomProtocolOpenAIEmbeddings:
			nativePlatform = PlatformOpenAI
		}
		if nativePlatform != "" {
			if err := s.rebuildBucketsForPlatform(ctx, nativePlatform, groupIDs, reason, seen); err != nil && firstErr == nil {
				firstErr = err
			}
		}
	}
...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	platforms := []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformCustom}
	var firstErr error
