...
def main() -> None:
    ...
    for needle in (
        ...
        "Mark generated VERSION artifact as intentional",
        "git status --short --untracked-files=no -- backend/cmd/server/VERSION",
        "git status --short --untracked-files=no -- . ':(exclude)backend/cmd/server/VERSION'",
        "git update-index --assume-unchanged backend/cmd/server/VERSION",
