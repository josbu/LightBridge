# LightBridge Deployment Files

This directory contains files for deploying LightBridge on Linux servers.
...
| `LightBridge.service` | Systemd service unit file |
| `LightBridge-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/WilliamWang1721/LightBridge/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
# View logs
docker compose -f docker-compose.local.yml logs -f LightBridge
...
```bash
# Clone repository
git clone https://github.com/WilliamWang1721/LightBridge.git
cd LightBridge/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs LightBridge | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/LightBridge-datamanagement.sock`
...
# Restart LightBridge only
docker compose -f docker-compose.local.yml restart LightBridge
...
# View logs
docker compose logs -f LightBridge

# Restart LightBridge only
docker compose restart LightBridge
...
| `ADMIN_EMAIL` | No | `admin@LightBridge.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf LightBridge-complete.tar.gz deployment/

# Transfer to new server
scp LightBridge-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf LightBridge-complete.tar.gz
cd deployment/
docker compose -f docker-compose.local.yml up -d
```
...
LightBridge supports three methods to connect to Gemini:
...
# GEMINI_CLI_OAUTH_CLIENT_ID=your-google-oauth-client-id
# GEMINI_CLI_OAUTH_CLIENT_SECRET=your-google-oauth-client-secret
```
...
1. Download the latest release from [GitHub Releases](https://github.com/WilliamWang1721/LightBridge/releases)
2. Extract and copy the binary to `/opt/LightBridge/`
3. Copy `LightBridge.service` to `/etc/systemd/system/`
4. Run:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable LightBridge
   sudo systemctl start LightBridge
   ```
...
# Stop the service
sudo systemctl stop LightBridge
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit LightBridge
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/LightBridge.service
   ```
...
2. Add your OAuth credentials in the `[Service]` section (after the existing `Environment=` lines):
   ...
   ```ini
   Environment=GEMINI_CLI_OAUTH_CLIENT_ID=your-google-oauth-client-id
   Environment=GEMINI_CLI_OAUTH_CLIENT_SECRET=your-google-oauth-client-secret
   ```
...
The main config file is at `/etc/LightBridge/config.yaml` (created by Setup Wizard).
...
```
/opt/LightBridge/
├── LightBridge              # Main binary
├── LightBridge.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 LightBridge
...
# View detailed logs
docker compose logs --tail=100 LightBridge
...
# View recent logs
sudo journalctl -u LightBridge -n 50

# Check config file
sudo cat /etc/LightBridge/config.yaml
...
LightBridge supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.LightBridge.org](https://tls.LightBridge.org/)** to get TLS fingerprint information for different devices and browsers.
