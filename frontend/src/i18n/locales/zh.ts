export default {
  ...
  admin: {
    ...
    users: {
      ...
      lookupApiKeyOwner: '查找 Key 所属用户',
      apiKey: 'API Key',
      apiKeyLookupPlaceholder: '输入完整 API Key',
      apiKeyRequired: '请输入 API Key',
      apiKeyOwnerNotFound: '未找到 API Key 所属用户',
      lookup: '查找',
      group: '分组',
      none: '无',
      ...
      // 批量操作
      bulkEdit: '批量修改',
      bulkEditTitle: '批量修改 {count} 个用户',
      bulkEditHint: '勾选要修改的字段，未勾选的字段保持不变。',
      bulkEditSuccess: '已成功更新 {count} 个用户',
      bulkEditPartial: '部分成功：成功 {success} 个，失败 {failed} 个',
      bulkEditConcurrencyPlaceholder: '输入并发数（例如 10）',
      bulkEditNotesPlaceholder: '输入备注内容',
      bulkDeleteConfirm: '确定要删除选中的 {count} 个用户吗？此操作无法撤销。',
      bulkDeleteSuccess: '已删除 {count} 个用户',
    ...
    accounts: {
      ...
      dataActions: '数据操作',
      toolActions: '工具',
      repairOpenAIOAuthPlatform: '修复 OpenAI OAuth 平台',
      repairOpenAIOAuthPlatformConfirm: '将扫描已被标记为 Gemini OAuth、但凭据属于 OpenAI OAuth 的账号，并改回 OpenAI。此操作不会修改真正的 Gemini OAuth 账号。是否继续？',
      repairOpenAIOAuthPlatformCompleted: '修复完成：已修复 {repaired} 个账号，扫描 {scanned} 个 Gemini OAuth 账号。',
      repairOpenAIOAuthPlatformCompletedWithErrors: '修复完成但有错误：已修复 {repaired}，失败 {failed}，扫描 {scanned}。',
      repairOpenAIOAuthPlatformNoChanges: '未发现需要修复的 OpenAI OAuth 账号。扫描 {scanned} 个 Gemini OAuth 账号。',
      repairOpenAIOAuthPlatformFailed: '修复 OpenAI OAuth 平台失败',
      viewColumns: '列显示',
      ...
      dataImportCompatibilityModeHint: '用于非标准 JSON/TXT，系统会自动提取 refresh_token、access_token、id_token、session_token、邮箱和账号 ID 等字段并导入为 OpenAI OAuth 账号。',
      ...
      testConnection: '测试连接',
      verifyAuthenticity: '检测真伪',
      verifyAuthenticityDone: '真伪检测完成：{verdict}',
      verifyAuthenticityFailed: '真伪检测失败',
      reAuthorize: '重新授权',
      refreshToken: '刷新令牌',
      noAccountsYet: '暂无账号',
      ...
      authenticity: {
        ...
        unknown: '未知',
        checkedAt: '检测时间',
        method: { probe: '主动探针', passive: '被动检测' }
      },
      resetStatus: '重置状态',
      statusReset: '账号状态已重置',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 LightBridge 实例的场景。',
      ...
      gemini: {
        ...
        proxyManagedHint: '此接入类型由 LightBridge 自动拉起 aistudio-api 反代子进程。无需手动填写上游地址与 Token；只需粘贴一次 Google 登录 Cookie 即可绑定会话。首次发起请求时 LB 会自动启动进程。',
        proxyCookieLabel: 'Google 登录 Cookie',
        proxyCookiePlaceholder: '从浏览器复制 Google 的 Cookie 串（如 SID=...; __Secure-1PSID=...; ...）',
        proxyCookieHint: '访问 https://myaccount.google.com 登录后，用浏览器扩展导出 Cookie 粘贴至此。Cookie 有效期较短，过期后需重新粘贴。',
        proxyCookieBound: 'AIStudio 反代账号的 Google 会话已绑定。',
        proxyCookieBindFailed: '绑定 Cookie 失败：',
        proxyRuntimeTitle: '运行时环境',
        proxyRuntimeCheck: '检测',
        proxyRuntimeChecking: '检测中…',
        proxyRuntimeReady: '运行时已就绪（Python + aistudio-api + 浏览器）。',
        proxyRuntimeNotReady: '运行时未就绪，需要安装。',
        proxyRuntimeInstallHint: '点击下方按钮自动安装 aistudio-api 及其依赖；Linux 系统库缺失时需按提示手动执行 apt。',
        proxyRuntimeInstall: '一键安装',
        proxyRuntimeInstalling: '安装中…请勿关闭页面',
        proxyRuntimeInstallDone: '运行时安装完成。',
        proxyRuntimeInstallFailed: '运行时安装失败。',
        proxyBindModeLabel: 'Google 账号绑定方式',
        proxyBindModeCookie: '粘贴 Cookie',
        proxyBindModeLogin: '引导登录',
        proxyBindModeHint: '引导登录会弹出有头浏览器窗口（需宿主有 GUI/xvfb）；无头环境请用粘贴 Cookie。',
        proxyLoginHint: '创建账号后将自动弹出浏览器窗口，请在其中完成 Google 登录。登录完成后会话会被自动捕获。',
        proxyLoginStarted: '已启动浏览器登录，请在弹出的窗口完成 Google 登录…',
        proxyLoginDone: 'Google 登录完成，会话已绑定。',
        proxyLoginFailed: '引导登录失败：',
        proxyBaseUrlHint: '填写 aistudio-api 反代服务地址（如 http://host:8080）；若为 http 需在系统设置中允许非 HTTPS 上游。',
        proxyTokenHint: 'aistudio-api 中配置的鉴权 Token（secret_token）。',
        helpButton: '使用帮助',
        helpDialog: {
          title: 'Gemini 使用指南',
      ...
      lightBridgeConnect: {
        ...
        refreshBalance: '刷新余额',
        refreshBalanceSuccess: '余额已刷新',
        refreshBalanceFailed: '余额刷新失败',
        openConsole: 'New API 控制台',
        consoleUrlMissing: '该账号缺少 New API 实例地址，无法打开控制台',
        userId: 'New API 用户 ID',
        userIdPlaceholder: '例如：1',
        userIdHint: '您的 New API 数字用户 ID，作为 New-Api-User 请求头发送（必填）。在 New API「个人设置」页可查看。',
        systemToken: '系统访问令牌',
    ...
    settings: {
      ...
      features: {
        ...
        privacyFilter: {
          ...
          configureLink: '前往 隐私过滤 配置脱敏规则',
          enabled: '启用隐私过滤',
          enabledHint: '关闭后管理员侧边栏入口隐藏，网关请求/响应脱敏不会执行。',
        },
        affiliate: {
          title: '邀请返利',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 LightBridge 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 LightBridge 用户登录',
      ...
      authenticity: {
        title: 'Claude 模型真伪检测',
        description: '通过校验 thinking 签名，检测 Claude/Anthropic 账号是否为正版原生',
        enabled: '启用被动检测',
        enabledHint: '开启后，会对带 thinking 的请求响应做签名旁路检查，持续缺失合法签名的账号将被标记为可疑',
        threshold: '可疑次数阈值',
        thresholdHint: '连续多少次"开启 thinking 但无合法签名"才将账号标记为假冒（避免上游临时降级误判）',
        saved: '真伪检测设置保存成功',
        saveFailed: '保存真伪检测设置失败'
      },
      betaPolicy: {
        title: 'Beta 策略',
